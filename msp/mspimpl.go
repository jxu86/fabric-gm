...
import (
	...
	"github.com/jxu86/fabric-gm/bccsp/signer"
...
func (msp *bccspmsp) getUniqueValidationChain(cert *sm2.Certificate, opts sm2.VerifyOptions) ([]*sm2.Certificate, error) {
	...
	// fmt.Println("optsbytes===>", string(optsbytes))

	// certJson, _ := json.Marshal(cert)
	// fmt.Println("certJson===>", string(certJson))

	validationChains, err := cert.Verify(opts)
	// fmt.Println("##validationChains==>", validationChains)
	// fmt.Println("##err==>", err)
	if err != nil {
