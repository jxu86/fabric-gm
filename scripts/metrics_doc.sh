...
generate_doc() {
    local gendoc_command="go run github.com/jxu86/fabric-gm/common/metrics/cmd/gendoc"
    local orderer_prom
    ...
    local orderer_deps=()
    while IFS= read -r pkg; do orderer_deps+=("$pkg"); done < <(go list -deps github.com/jxu86/fabric-gm/cmd/orderer | sort -u | grep hyperledger)
    ...
    local peer_deps=()
    while IFS= read -r pkg; do peer_deps+=("$pkg"); done < <(go list -deps github.com/jxu86/fabric-gm/cmd/peer | sort -u | grep hyperledger)
